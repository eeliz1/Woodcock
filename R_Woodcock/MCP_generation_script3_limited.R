...
for(fileName in fileNames){
  ...
  df$dt=as.POSIXct(df$gmt-(60*60*6))
  df=df[order(df$dt),]
...
##############################################################################################################    
      ...
      dfp2 = subset(dfp, format (dfp$dt, '%H') %in% hrs1)
      
#      coordinates(dfp2)=c("Longitude", "Latitude")
#      proj4string(dfp2)<-CRS('+proj=longlat +datum=WGS84')
